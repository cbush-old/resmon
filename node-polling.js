 ...
 */
// Somewhat hacked for naivebs: send response headers
(function(){
    ...
    HttpConnMgt.prototype.SendByUid = function(uid, dataStr, responseProps){
        if(this._httpConnAry[uid]){
            let res = this._httpConnAry[uid].res;
            for (let prop in responseProps) {
                res[prop] = responseProps[prop];
            }
            res.end(dataStr);
